...
def test_indexing(comparison: Comparison) -> None:
    ...
    assert set(xs_indexed) | set(ys_indexed) == set(index.get_indexer(pd.Index(index.tolist())))
...
def test_llmfao_pairwise_scores(
    ...
    func = partial(evalica.pairwise_scores, np.asarray(result.scores, dtype=np.float64), solver=solver)
